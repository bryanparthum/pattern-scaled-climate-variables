...
files <- list.files('patterns', full.names=T) %>%
...
pattern = raster(files[i], varname="pattern") %>% 
  rotate()

## country polygons
world <- st_as_sf(cleangeo::clgeo_Clean(getMap()[-which(getMap()$ADMIN=='Antarctica'),])) %>% 
...
if (SSP=='ssp1') {
  ...
  mur = st_read('murakami/grid.shp') %>% 
    st_transform(st_crs(pattern))
  gdp = read_csv(paste0('murakami/gdp_',SSP,'.csv')) %>% 
    dplyr::select(gID,g1_2000,g1_2100)
  pop = read_csv(paste0('murakami/pop_',SSP,'.csv')) %>% 
  ...
  ## downscaled economic data: http://www.cger.nies.go.jp/gcp/population-and-gdp.html
  mur = 
    st_read('data/downscaling/murakami/grid.shp') %>% 
    st_transform(st_crs(pattern))
  gdp = 
    read_csv(paste0('data/downscaling/murakami/gdp_',SSP,'.csv')) %>% 
    dplyr::select(gID,g2_2000,g2_2100)
  pop = 
    read_csv(paste0('data/downscaling/murakami/pop_',SSP,'.csv')) %>% 
    dplyr::select(gID,p2_2000,p2_2100)
  mur = 
    left_join(left_join(mur,gdp),pop) 
  rm(gdp,pop)
  ## country temp patterns
  ...
  ## downscaled economic data: http://www.cger.nies.go.jp/gcp/population-and-gdp.html
  mur = st_read('murakami/grid.shp') %>% st_transform(st_crs(pattern))
  gdp = read_csv(paste0('murakami/gdp_',SSP,'.csv')) %>% 
...
p = ggarrange(top=paste0(substr(str_split(files[i],'/')[[1]][[4]],17,nchar(str_split(files[i],'/')[[1]][[4]])-10)),
               ...
               world %>% ggplot() +
                 ...
                 theme(axis.text=element_text(size=12),
                       panel.grid.major.x = element_blank(),
                       panel.grid.minor.x = element_blank()))
ggsave(paste0('figures/cmip6/',SSP,'_2100_',substr(str_split(files[i],'/')[[1]][[4]],17,nchar(str_split(files[i],'/')[[1]][[4]])-10),'.png'),p,width=8.5,height=11)

patterns = 
  bind_rows(patterns,world %>% 
                       st_drop_geometry() %>% 
                       mutate(scenario=SSP,source=substr(str_split(files[i],'/')[[1]][[4]],17,nchar(str_split(files[i],'/')[[1]][[4]])-10),source.id=i))
...
patterns %<>% 
  bind_rows(.,pat %>% 
              ...
              mutate(scenario=case_when(scenario=='ssp1'~'ssp5',scenario=='ssp2'~'ssp4'))) %>% 
  arrange(source.id,scenario,iso3)

## save
write_csv(patterns,'cmip6_pattern_scaling_by_country.csv')

## collapse to an average pattern across cmip models by country
write_csv(patterns %>% select(-c(source,source.id)) %>% 
            group_by(name,iso3,continent,scenario) %>% 
            summarise_all(mean,na.rm=T),
          'cmip6_pattern_scaling_by_country_mean.csv')
